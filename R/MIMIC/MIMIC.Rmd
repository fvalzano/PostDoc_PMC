...
base_directory = "/hpc/pmc_kool/fvalzano/Rstudio_Test1/MIMIC/Data/10x_runs/" #Make sure you have the file you want to analyze in this folder: Name the folder with sequencing facility ID and put filtered_feature_bc_matrix in it, the script will fetch the data that it needs
...
## Metadata curation
```{r MimicID}
#Add MimicID from excel sheet Metadata.xlsx
...
```
```{r Cancer type}
#Add Cancer type from excel sheet Metadata.xlsx
scrna_mimic = read_rds("/hpc/pmc_kool/fvalzano/Rstudio_Test1/MIMIC/Data/All/scrna_mimic_all.rds")
metadata_ID = readxl::read_xlsx("/hpc/pmc_kool/fvalzano/Rstudio_Test1/MIMIC/Metadata.xlsx", sheet = 2)
...
```
## Trajectory + pseudotime - IMPORTANT: Not available in VSC, run on JobHopper
```{r Trajectory and Pseudotime by monocle3}
scrna_mimic = read_rds("/hpc/pmc_kool/fvalzano/Rstudio_Test1/MIMIC/Data/All/scrna_mimic_all.rds")
#convert seurat object in cds object using the specific wrapper
cds_mimic = as.cell_data_set(scrna_mimic)
#partitions:
##IMPORTANT: in order to run monocle3 we have to split the object by partitions, that split the dataset according to the capability of finding a connecting trajectory or not in the dataset
#Re-cluster the cells
cds_mimic <- cluster_cells(cds_mimic, resolution=1e-3)
#Create a new object containing the tumoral cells, corresponding to partition 1, in this case
scrna_mimic_sub <- subset(as.Seurat(cds_mimic, assay = NULL), monocle3_partitions == 1)
#subset the original seurat object using the barcodes from the subsetted (by partition) object
scrna_mimic_sub = subset(scrna_mimic[,colnames(scrna_mimic)%in%colnames(scrna_mimic_sub)])
#Reconvert the seurat object into cds data
cds_mimic = as.cell_data_set(scrna_mimic_sub)

#IMPORTANT: At this point you will have a subsetted object with only tumoral cells (according to the division by partition)
#Transfer annotation from the seurat object onto the cds object
...
#Learn trajectories in the dataset, use learn_graph_control=list(ncenter=1000) to control "branchyness" (https://github.com/cole-trapnell-lab/monocle3/issues/174)
...
#order cells to calculate pseudotime ordering
...
```{r Integration Fetal}
#Create integrated object with kaesmann cerebellar fetal development dataset to highlight shared cell types
scrna_mimic=read_rds("/hpc/pmc_kool/fvalzano/Rstudio_Test1/MIMIC/Data/All/scrna_mimic_all.rds")
scrna_kaesmann = readRDS(file = "/hpc/pmc_kool/fvalzano/Rstudio_Test1/Cerebellum Development/Kaesmann/seurat.rds")
#Some trouble with data slot of kaesmann dataset, raw counts are placed in the data slot instead of counts slot
scrna_kaesmann@assays[["RNA"]]@counts = scrna_kaesmann@assays[["RNA"]]@data  
#Balance the number of cells according to mimic dataset cell number
scrna_kaesmann_subsampled <- scrna_kaesmann[, sample(colnames(scrna_kaesmann), size =ncol(scrna_mimic), replace=F)]
#Prepare list for for loop
raw_reads = list()
seurat_objects=list()
scrna_matrix = list()
matrix_list = list()
...
#Split kaesmann dataset by batch(single samples)
...
for (i in unique(scrna_kaesmann_subsampled$batch)) {
 ...
 #genes in kaesmann dataset are annotated according ENSG instead of Gene Symbol, convert using list of GeneSymbol in metafeatures slot
...
#Convert IDs vector from single samples to transfer annotations from Kaesmann dataset to the new integrated Malignant+fetal dataset
IDs[stringr::str_detect(IDs, pattern = "SN")] = "Fetal"
IDs[stringr::str_detect(IDs, pattern = "LX")] = "Malignant" 
meta = as.data.frame(scrna_integrated$orig.ident)
meta$new.idents = ifelse(stringr::str_detect(meta$`scrna_integrated$orig.ident`, pattern = "SN"), "Fetal", "Malignant")
...
## Copy Number Variation - IMPORTANT: Not available in VSC, run on JobHopper
...
Idents(scrna_integrated) = "" #Annotation slot, work in progress
#set up elements for InferCNV analysis
counts_matrix = GetAssayData(scrna_integrated, layer="counts")
#Split object in Fetal and Malignant dataset 
scrna_integrated_split = SplitObject(scrna_integrated, split.by = "new.idents")
#Create metadata slot containing the name of the cluster + "_Fetal" or "_Malignant"
scrna_integrated_split$Fetal$anno_infercnv = as.factor(paste0(Idents(scrna_integrated_split$Fetal), "_Fetal"))
scrna_integrated_split$MB$anno_infercnv = as.factor(paste0(Idents(scrna_integrated_split$MB), "_Malignant"))
#Extract the barcodes for both the Fetal and Malignant datasets and then join them in one
barcodes = list()
...
#Extract the annotations for both the Fetal and Malignant datasets and then join them in one
annotation = list()
for (i in seq_along(scrna_integrated_split)) {
...
#IMPORTANT: after joining the annotation lists convert them into matrix, otherwise ERROR
annotation = as.matrix(annotation)
#Create infercnv object using as reference the Fetal counterpart of each cluster
...
scrna_integrated = infercnv::add_to_seurat(seurat_obj = scrna_integrated,
                                           ...
                                           top_n = 15)
write_rds(infercnv_integrated, "/hpc/pmc_kool/fvalzano/Rstudio_Test1/MIMIC/Data/All/infercnv/CNV_output_xCelltype/scrna_integrated_infercnv.rds")
```
...
annotation = as.matrix(annotation)
#Create infercnv object using as reference the Fetal counterpart of the whole dataset
infercnv_mimic = CreateInfercnvObject(raw_counts_matrix=counts_matrix,
                                    annotations_file=annotation,
                                    delim="\t",
...
filenames = c(paste0("LX", 430:432), paste0("LX", 434:436), paste0("LX", 438), paste0("LX", 440:441), paste0("LX", 500:504),paste0("LX", 507)) #Load the correct file labels
raw_reads = list() #Prepare raw reads list for for loop
seurat_objects=list() #Prepare seurat objects list for loop
...
#Use kaesmann precisest label metadata slot (highest granularity) as a reference for autoannotation
...
Idents(scrna_integrated) = "orig.ident"
#Convert IDs vector from single samples to transfer annotations from Kaesmann dataset to the new integrated MB+fetal dataset
IDs[stringr::str_detect(IDs, pattern = "SN")] = "Fetal"
IDs[stringr::str_detect(IDs, pattern = "LX")] = "MB"
meta = as.data.frame(scrna_integrated$orig.ident)
